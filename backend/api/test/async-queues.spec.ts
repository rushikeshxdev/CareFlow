...
import { QueueProducerService } from '../src/infrastructure/queues/queue-producer.service';
import { NotificationProcessor } from '../src/infrastructure/queues/processors/notification.processor';
import { ReminderProcessor } from '../src/infrastructure/queues/processors/reminder.processor';
...
async function runAsyncQueuesTestSuite() {
  console.log('🧪 Starting CareFlow Async Processing, Notifications & Reliability Test Suite...\n');
  ...
  try {
    ...
    if (jobResult.status !== 'CREATED' && jobResult.status !== 'SKIPPED_DUPLICATE') {
      throw new Error(`Expected NotificationProcessor to return CREATED or SKIPPED_DUPLICATE, got ${jobResult.status}`);
    }
    console.log(`  ✅ Notification worker processed job asynchronously (status: ${jobResult.status}).`);
    ...
    const verifiedAppt: any = await prisma.appointment.findUnique({
      where: { id: apptId },
      include: { careEvents: true } as any,
    });

    if (verifiedAppt?.status !== AppointmentStatus.CONFIRMED) {
      throw new Error('Appointment status is not CONFIRMED!');
    }
    if (!verifiedAppt?.careEvents || verifiedAppt.careEvents.length === 0) {
    ...
    const notifCount = await (prisma.notification as any).count({
    ...
    console.log('\n🎉 ALL ASYNC QUEUES, NOTIFICATIONS & RELIABILITY TESTS PASSED PERFECTLY!\n');
  } catch (error: any) {
